...
def get_word_score(word, n):
    ...
    """
    word = word.lower()
    scrabble_word_value = 0
    for char in word:
        scrabble_word_value = scrabble_word_value + SCRABBLE_LETTER_VALUES[char]
    word_length = len(word)

    on_hand = n - word_length
    score = (7 * word_length - 3 * on_hand)
    if score < 1:
        score = 1
    return score * scrabble_word_value
...
def update_hand(hand, word):
    ...
    """
    new_hand = hand.copy()
    guess = str.lower(word)
    for key in new_hand.keys():
        for i in range(len(guess)):
            if key in guess[i]:
                new_hand[key] = new_hand[key] - 1
    return new_hand
    ...
    * The hand finishes when there are no more unused letters.
    ...
    while True:
        if calculate_handlen(hand) == 0:
            print('Ran out of letters. Total score for this hand: %s' % total_score)
            break
        current_hand = display_hand(hand)
        print('Current hand: %s' % current_hand)
        get_input = str.lower(input('Please enter your word, or enter "!!" if finished: '))
        if get_input == '!!':
            print('Total score for this hand: %s' % total_score)
            break
        else:
            if is_valid_word(get_input, hand, word_list):
                current_score = get_word_score(get_input, calculate_handlen(hand))
                total_score = current_score + total_score
                hand = update_hand(hand, get_input)
                print("%s earned %s points. Total: %s points" % (get_input, current_score, total_score))
            else:
    ...
    * Returns the total score for the series of hands
    ...
    while number_hands >= 1:
        ...
        while substitute_count > 0:
            current_hand = display_hand(hand)
            print("Current hand: %s" % current_hand)
            sub_option = input('Would you like to substitute a letter: [y/n]')
        ...
        while replay_count >= 1:
            print('_________')
            replay_option = input('Would you like to replay the hand: [y/n]')
            if replay_option.lower() == 'y':
                while substitute_count >= 1:
                    sub_option = input('Would you like to substitute a letter: [y/n]')
                    if sub_option.lower() == 'y':
    ...
    print("Total score for all hands: %s" % total)
